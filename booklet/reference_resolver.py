...
def resolve_sunday_reference(query: str, today: date | None = None) -> SundayResolution:
    ...
    if relative := _match_relative_reference(normalized, scoped_entries=entries, today=today):
        return _resolved(query, relative)
...
def _match_relative_reference(
    text: str,
    scoped_entries: tuple[SundayEntry, ...],
    today: date,
) -> ResolvedSunday | None:
    target: date | None = None
    if any(token in text for token in ("this sunday", "this week", "upcoming sunday")):
        target = _next_sunday_on_or_after(today)
    elif any(token in text for token in ("next sunday", "next week")):
        target = _next_sunday_on_or_after(today + timedelta(days=1))

    if not target:
        return None

    for entry in scoped_entries:
        if entry.service_date == target:
            return _resolved_from_entry(entry)
    return None


def _next_sunday_on_or_after(day: date) -> date:
